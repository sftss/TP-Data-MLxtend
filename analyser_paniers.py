...
if __name__ == "__main__":
    ...
    if df_clean is not None:
        ...
        if not df_filtered.empty:
            ...
            une_heure_avant = max_date - timedelta(hours=1)
            print(f"Période analysée : {une_heure_avant} à {max_date}")

            df_derniere_heure = df_filtered[df_filtered['date'] >= une_heure_avant].copy()
            print(f"{df_derniere_heure.shape[0]} paniers pour MLxtend")
